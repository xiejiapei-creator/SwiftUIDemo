...
struct HierarchyRelatedView: View
{
    ...
    @State var showChoiceAlert = false
    ...
    var body: some View
    {
        TabView
        {
                        ...
                        HStack
                        {
                            ...
                            { index in
                                ...
                                { geometry in
                                    VStack
                                    {
                                        let distAbs = abs(fullView.frame(in: .global).midX - geometry.frame(in: .global).midX)
                                        ...
                                        // 缩小图片，选中的不缩小，其余图片x\y轴各自缩小为0.7倍
                                        .scaleEffect(x: centered ? 1.0 : 0.7, y: centered ? 1 : 0.7)
            ...
            NavigationView
            {
                ...
                .navigationBarTitle("导航标题", displayMode: .automatic)
            ...
            VStack(spacing: 50)
            {
                ...
                {
                    ActionSheet(title: Text("要执行什么操作？"), message: Text("补充文字说明"), buttons: [.default(Text("添加至收藏夹")){}, .destructive(Text("删除项目")){}, .cancel()])
